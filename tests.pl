...
if (scalar(@ARGV) > 0) {
  ...
  my $FEATURE_TESTS = {
    't/modules/mod_xfer_mmap/sftp.t' => {
      order => ++$order,
      test_class => [qw(mod_sftp mod_xfer_mmap)],
    },

    't/modules/mod_xfer_mmap/tls.t' => {
      order => ++$order,
      test_class => [qw(mod_tls mod_xfer_mmap)],
    },
  };
